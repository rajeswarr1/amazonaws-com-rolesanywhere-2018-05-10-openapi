...
func ListprofilesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/profiles%s", cfg.BaseURL, queryString)
...
func CreateListprofilesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_profiles",
		mcp.WithDescription("<p>Lists all profiles in the authenticated account and Amazon Web Services Region.</p> <p> <b>Required permissions: </b> <code>rolesanywhere:ListProfiles</code>. </p>"),
