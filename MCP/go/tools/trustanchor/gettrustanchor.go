...
func GettrustanchorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGettrustanchorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_trustanchor_trustAnchorId",
		mcp.WithDescription("<p>Gets a trust anchor.</p> <p> <b>Required permissions: </b> <code>rolesanywhere:GetTrustAnchor</code>. </p>"),
		mcp.WithString("trustAnchorId", mcp.Required(), mcp.Description("The unique identifier of the trust anchor.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GettrustanchorHandler(cfg),
	}
}
