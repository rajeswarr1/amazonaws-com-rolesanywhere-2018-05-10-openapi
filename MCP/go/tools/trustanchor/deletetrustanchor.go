...
func DeletetrustanchorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletetrustanchorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_trustanchor_trustAnchorId",
		mcp.WithDescription("<p>Deletes a trust anchor.</p> <p> <b>Required permissions: </b> <code>rolesanywhere:DeleteTrustAnchor</code>. </p>"),
		mcp.WithString("trustAnchorId", mcp.Required(), mcp.Description("The unique identifier of the trust anchor.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletetrustanchorHandler(cfg),
	}
}
