...
func GetcrlHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetcrlTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_crl_crlId",
		mcp.WithDescription("<p>Gets a certificate revocation list (CRL).</p> <p> <b>Required permissions: </b> <code>rolesanywhere:GetCrl</code>. </p>"),
		mcp.WithString("crlId", mcp.Required(), mcp.Description("The unique identifier of the certificate revocation list (CRL).")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetcrlHandler(cfg),
	}
}
