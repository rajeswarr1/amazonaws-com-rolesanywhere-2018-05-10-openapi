...
func DeletecrlHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletecrlTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_crl_crlId",
		mcp.WithDescription("<p>Deletes a certificate revocation list (CRL).</p> <p> <b>Required permissions: </b> <code>rolesanywhere:DeleteCrl</code>. </p>"),
		mcp.WithString("crlId", mcp.Required(), mcp.Description("The unique identifier of the certificate revocation list (CRL).")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletecrlHandler(cfg),
	}
}
